...
PKT_TYPE_FRAME_HEADER = 3   # NEW in Day 6
...
# ---------- Chunking helpers (unchanged from Day 5) ----------

def chunk_patch_bytes(
    data: bytes, chunk_size: int = MAX_PAYLOAD_BYTES,
) -> List[bytes]:
    if chunk_size <= 0:
...
def build_packets(
    *,
    ...
    for idx, chunk in enumerate(chunks):
        hdr = PacketHeader(
            frame_id=frame_id, det_id=det_id, quality=quality,
            chunk_idx=idx, chunk_count=total, payload_len=len(chunk),
